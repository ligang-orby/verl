...
def convert_bbox_xywh_to_xyxy(bbox_xywh: tuple[float, float, float, float]) -> tuple[float, float, float, float]:
    """
...
def random_crop(image, safe_box: tuple[float, float, float, float], min_size: int = 334):
    ...
    image = image.crop((offset_x, offset_y, offset_x + image_width, offset_y + image_height))
